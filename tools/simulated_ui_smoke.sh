...
process_id=""
video_path="${SFM_SMOKE_VIDEO:-${SFM_P119_VIDEO:-$root_dir/模擬器/測試影片/720p/P1190119_720p.MP4}}"
...
SFM_WORKSPACE_ROOT="$root_dir" setsid \
  ...
  --map "$root_dir/地圖檔/場域/river_site/releases/river_gluemap_all8_direct_20260908/map/map.ply" \
  --video "$video_path" \
  >"$log_file" 2>&1 &
process_id=$!
...
for _ in $(seq 1 120); do
  ...
  if grep -Fq "BOOT_INIT: live MegaLoc/PnP locked" "$log_file"; then
    boot_ready=1
  fi
  ...
  if [[ "$feed_ready" == "1" && "$pose_ready" == "1" && "$boot_ready" == "1" ]]; then
    break
  fi
  if ! kill -0 "$process_id" 2>/dev/null; then
    break
...
if [[ "$boot_ready" != "1" ]]; then
  echo "[ui-smoke] simulated GUI did not release its initial frame hold" >&2
  exit 1
fi
