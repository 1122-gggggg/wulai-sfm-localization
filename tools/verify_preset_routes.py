...
def _phase_counts(trace):
    ...
    for tick in trace:
        ...
        if phase == "translate" and yaw != 0 and gaz == 0 and abs(int(yaw)) <= 20:
            yaw_overlay_ok += 1
    return turn_yaw_gaz_overlap, centering_yaw, translate_gaz_ok, yaw_overlay_ok


def _measure_route_trace(result, controller, params, join):
    errors = []
    target_order = []
    ...
    turn_yaw_gaz_overlap, centering_yaw, translate_gaz_ok, yaw_overlay_ok = _phase_counts(
        result.trace
    )
    for tick in result.trace:
        ...
        if retired is not None and retired not in seen_retired:
            ...
            limit = float(controller._arrive_radius_for(int(retired))) + 3.0 * float(
                params.pos_noise_u
            )
            ok = dist <= limit
            arrival_detail.append(
                {"retired": int(retired), "truth_distance_u": dist, "limit_u": limit, "ok": ok}
            )
            if not ok:
                arrival_truth_ok = False
...
def _tracking_failure_reason(
    result, controller, params, p95, maximum, p95_limit, maximum_limit, end_distance
):
    details = []
    ...
    return "; ".join(details) if details else result.reason
...
def evaluate(params: sim.SimParams, *, start_label: str) -> dict:
    ...
    if not result.trace:
        return {
            ...
            "reason": "empty trace: no samples to verify",
            "missing_truth_fields": missing or None,
            "post_join_error_p95_u": None,
            "post_join_error_max_u": None,
            "sequence_ok": False,
    ...
    if missing:
        return {
            ...
            "accepted": False,
            "completed": result.success,
            "reason": f"trace missing truth fields: {sorted(set(missing))}",
    ...
    (
        ...
        translate_gaz_ok,
        yaw_overlay_ok,
    ) = _measure_route_trace(result, controller, params, join)
    ...
    accepted_target_orders = (
        [expected, expected[1:]] if retired_order and retired_order[0] == 0 else [expected]
    )
    sequence_ok = (
    ...
    if result.trace:
        ...
        try:
            last_speed = last.get(
                "landing_speed_measured_mps", last.get("ground_speed_mps", float("inf"))
            )
            landing_ok = bool(last.get("action") == "LAND" or result.success) and float(
                last_speed
            ) <= float(LANDING_SPEED_THRESHOLD_MPS)
        except (TypeError, ValueError, OverflowError):
            landing_ok = False
    accepted = bool(
        result.success
    ...
    if not sequence_ok:
        ...
        reason = _tracking_failure_reason(
            result, controller, params, p95, maximum, p95_limit, maximum_limit, end_distance
        )
    elif contract_bad:
...
def _gust_recoveries(result, controller):
    recoveries = []
...
def evaluate_gust_recovery(params: sim.SimParams) -> dict:
    ...
    expected = list(range(len(controller.wp)))
    recoveries = _gust_recoveries(result, controller)
    turn_bad, center_bad, _translate_gaz, _yaw_overlay = _phase_counts(result.trace)
    terminal_ok = False
    ...
    accepted = bool(
        ...
        and recoveries
        and all(
            item["recovery_s"] is not None and item["recovery_s"] <= 5.0 for item in recoveries[:-1]
        )
        and (
            recoveries[-1]["recovery_s"] is not None
            and recoveries[-1]["recovery_s"] <= 5.0
            or terminal_ok
        )
    )
    reason = result.reason
    if not result.success:
        ...
        slow = next(
            item
            for item in recoveries[:-1]
            if item["recovery_s"] is None or item["recovery_s"] > 5.0
        )
...
def _record_waypoint_starts(controller, frame, doc, route, args, record):
...
def _record_tilt_starts(controller, frame, doc, route, record):
...
def _record_drift_starts(controller, frame, doc, route, record):
    ...
    if leg is not None:
        ...
        drift = normal[0] * np.asarray(frame.east, float) + normal[1] * np.asarray(
            frame.north, float
        )
        for index in (0, len(doc.waypoints) - 1):
            for sign in (1.0, -1.0):
...
def _record_fault_starts(controller, frame, doc, route, routes, record):
...
def _record_route_starts(route, routes, args, record):
    controller, _config, frame, doc = sim.build_production_controller(
        route, sim._default_align(), return_to_start=True
    )
    _record_waypoint_starts(controller, frame, doc, route, args, record)
    if not args.quick:
        _record_tilt_starts(controller, frame, doc, route, record)
        _record_drift_starts(controller, frame, doc, route, record)
        ...
        _record_fault_starts(controller, frame, doc, route, routes, record)
        for index in (0, len(doc.waypoints) - 1):
            for seed in (101, 137):
                offset = (
                    controller.wp[index] - controller.wp[0] + 0.09 * frame.east + 0.02 * frame.up
                )
                record(
                    sim.SimParams(
                        route=route,
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        type=float,
        required=True,
        help=f"initial localization wait included in the {sim.AUTO_MAX_DURATION_S:g}s mission budget",
    )
    ...
    for route in routes:
        _record_route_starts(route, routes, args, record)
        print(
