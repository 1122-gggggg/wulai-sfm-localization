...
def main() -> int:
    ...
    if path is None:
        print(
            "[bench] FAIL: 找不到 SkyController joystick 節點 "
            "(/dev/input/by-id/usb-Parrot*Skycontroller*-joystick 或 /dev/input/js*)"
        )
        print(
            "[bench] 遙控器沒開或沒插 USB 時,backend 會進 STICK_MONITOR_FAIL,"
            "起飛會被 _takeoff_preflight 擋下。"
        )
        return 1
    print(f"[bench] joystick 節點: {path}")
    print(
        f"[bench] deadzone = {_STICK_DEADZONE} (實測滿舵約 +/-28715, "
        f"即可用行程的 {_STICK_DEADZONE / 28715 * 100:.0f}%)"
    )
    print(f"[bench] 可奪取控制權的飛行軸 = {list(_STICK_FLIGHT_AXES)}  (相機/雲台軸不列入)")
    ...
    def on_active(axes: dict[int, int]) -> None:
        ...
        detail = ", ".join(
            f"{AXIS_NAMES.get(a, f'axis{a}')}={v:+6d} ({abs(v) / 32767 * 100:.0f}%)"
            for a, v in sorted(moved.items())
        )
        print(
            f"[bench] {time.monotonic() - t0:5.1f}s  STICK_OVERRIDE -> "
            f"give_to_pilot()  |  {detail}",
            flush=True,
        )

    def on_disconnect(reason: str) -> None:
        print(
            f"[bench] {time.monotonic() - t0:5.1f}s  DISCONNECT: {reason}  "
            "-> backend 會標記 STICK_MONITOR_FAIL,空中則原地降落",
            flush=True,
        )
    ...
    print(f"[bench] axes_active(最後軸值) = {axes_active(snapshot)}  (放開後應為 False)")
    if not fired:
