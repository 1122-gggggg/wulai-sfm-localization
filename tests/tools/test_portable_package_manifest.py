...
def test_portable_export_keeps_output_governance_readme() -> None:
    ...
    assert {"pyproject.toml", "pytest.ini"}.issubset(exporter.COPY_FILES)
    assert "requirements" in exporter.COPY_DIRS
...
def _write_offline_wheelhouse(
    ...
    *,
    ...
    lock_records = [
        {"name": Path(relative).name, "sha256": _sha256(root / relative)}
...
def test_live_minimal_export_reuses_runtime_only_wheelhouse(
    ...
    runtime_locks = ("requirements/runtime-lock.txt",)
    runtime_lock_names = ("runtime-lock.txt",)
    ...
    monkeypatch.setattr(
        ...
        ("RUNTIME_ARTIFACTS.json", "requirements/runtime-lock.txt"),
    )
    ...
    def fake_subset(
        ...
        *,
        ...
        assert tuple(Path(path).name for path in requirement_locks) == runtime_lock_names
...
def test_portable_manifest_accepts_a_runtime_only_offline_wheelhouse(
    ...
    lock_relative = "requirements/runtime-lock.txt"
    lock_name = "runtime-lock.txt"
    lock_path = tmp_path / lock_relative
    lock_path.parent.mkdir()
    lock_path.write_text("runtime\n", encoding="utf-8")
    wheelhouse = _write_offline_wheelhouse(tmp_path, (lock_relative,))
