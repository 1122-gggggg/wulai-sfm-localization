...
def test_tilt_responds_symmetrically_on_both_axes_and_rotates_with_yaw():
    ...
    for _ in range(int(10.0 / sim.DT)):
        wind, _, _ = sim._integrate_plant(
            s_pos, wind, None, params, frame, rng, (20, 0, 0, 0), vh, vv, vy, 0.0, elapsed_s=5.0
        )
    ...
    for _ in range(int(10.0 / sim.DT)):
        wind, _, _ = sim._integrate_plant(
            s_neg, wind, None, params, frame, rng, (-20, 0, 0, 0), vh, vv, vy, 0.0, elapsed_s=5.0
        )
    ...
    for _ in range(int(10.0 / sim.DT)):
        wind, _, _ = sim._integrate_plant(
            s_fwd, wind, None, params, frame, rng, (0, 20, 0, 0), vh, vv, vy, 0.0, elapsed_s=5.0
        )
    fwd_axis = np.asarray(frame.east, float) * math.cos(0.0) + np.asarray(
        frame.north, float
    ) * math.sin(0.0)
    right_axis = np.asarray(frame.east, float) * math.sin(0.0) - np.asarray(
        frame.north, float
    ) * math.cos(0.0)
    assert abs(float(np.dot(s_pos.pos_m, right_axis))) == pytest.approx(
        abs(float(np.dot(s_fwd.pos_m, fwd_axis))), rel=1e-6
    )
    ...
    for _ in range(int(10.0 / sim.DT)):
        wind, _, _ = sim._integrate_plant(
            s_yaw, wind, None, params, frame, rng, (20, 0, 0, 0), vh, vv, vy, 0.0, elapsed_s=5.0
        )
    want = np.asarray(frame.east, float) * math.sin(math.pi / 2) - np.asarray(
        frame.north, float
    ) * math.cos(math.pi / 2)
...
def test_battery_sag_reduces_response_near_thrust_margin():
    ...
    for _ in range(int(10.0 / sim.DT)):
        w0, _, _ = sim._integrate_plant(
            s0, w0, None, fresh, frame, r0, (0, 50, 0, 50), vh, vv, vy, 0.0, elapsed_s=9.0
        )
        w1, _, _ = sim._integrate_plant(
            s1, w1, None, sagged, frame, r1, (0, 50, 0, 50), vh, vv, vy, 0.0, elapsed_s=9.0
        )
...
def test_fixed_ground_plane_ignores_hover_anchor_climb():
    ...
    params = sim.SimParams(
        ...
        duration_s=10.0,
        seed=7,
        ground_effect_frac=1.0,
        ground_altitude_m=0.0,
        ground_effect_height_m=1.0,
        tau_tilt_s=0.15,
    )
    ctrl, cfg, frame, _ = sim.build_production_controller(route, align, return_to_start=True)
...
def test_telemetry_delay_hides_samples_and_drop_keeps_stamp():
    ...
    assert float(
        sim_mod.SimParams(route=_route(), align=_align()).telemetry_latency_ms
    ) == pytest.approx(200.0)
    route, align = _route(), _align()
    params = sim.SimParams(
        route=route,
    ...
    assert all(
        r["measured_body_velocity_mps"] is None and r["telemetry_stamp"] is None
        for r in result.trace
    )
    params2 = sim.SimParams(
        route=route,
        align=align,
        duration_s=3.0,
        seed=7,
        telemetry_latency_ms=200.0,
        telemetry_drop_rate=0.0,
        wind_sigma_mps=0.0,
        pos_noise_u=0.0,
    )
    ...
    assert (
        min(
            r["t"] - r["telemetry_stamp"] for r in result2.trace if r["telemetry_stamp"] is not None
        )
        >= 0.15
    )
...
def test_speed_limiter_changes_output_in_sim():
    route, align = _route(), _align()
    params = sim.SimParams(
        route=route, align=align, duration_s=60.0, seed=7, wind_sigma_mps=0.0, speed_limit_mps=0.05
    )
    result, _ = sim.run_sim(params, record_trace=True)
...
def test_fault_families_recover_and_surface_in_trace():
    ...
    route, align = _route(), _align()
    base = dict(
        route=route, align=align, meters_per_unit=5.0, duration_s=300.0, seed=7, wind_sigma_mps=0.0
    )
    cases = [
...
def _regression_base(**kw):
    ...
    base = dict(
        route=route, align=align, meters_per_unit=15.0, duration_s=120.0, seed=7, wind_sigma_mps=0.0
    )
    base.update(kw)
...
def _fly_localizer(velocity_mps, t_end, t_start=0.0, **config):
    ...
    cfg = SimulatedLocalizerConfig(
        s=10.0,
        pos_sigma_u=0.0,
        yaw_sigma_deg=0.0,
        latency_ms=0.0,
        drop_rate=0.0,
        map_up_u=(0.0, -1.0, 0.0),
        **config,
    )
...
def test_drift_window_before_any_map_fix_has_no_pose():
    samples = _fly_localizer(
        (1.0, 0.0, 0.0), 2.0, t_start=1.0, drift_every_s=100.0, drift_dur_s=5.0
    )
