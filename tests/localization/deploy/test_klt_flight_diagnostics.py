...
def test_klt_recovers_known_image_translation_and_records_quality():
    ...
    points = points[
        (points[:, 0] > 64) & (points[:, 0] < 448) & (points[:, 1] > 64) & (points[:, 1] < 320)
    ]
    moved = cv2.warpAffine(
        original, np.float32([[1, 0, 3], [0, 1, -2]]), (512, 384), borderMode=cv2.BORDER_REFLECT101
    )
...
def test_pnp_debug_samples_are_bounded_timestamped_and_reprojectable():
    ...
    tracker.profile = SimpleNamespace(
        fast_loop=SimpleNamespace(pnp=SimpleNamespace(min_points=4, min_inliers=4))
    )
    tracker._w, tracker._h = 512, 384
    tracker._K = np.array([[300.0, 0, 256], [0, 300, 192], [0, 0, 1.0]])
    ...
    tracker._pycolmap = SimpleNamespace(
        estimate_and_refine_absolute_pose=lambda *a: {
            "inlier_mask": np.ones(200, bool),
            "cam_from_world": SimpleNamespace(matrix=lambda: pose),
        }
    )
