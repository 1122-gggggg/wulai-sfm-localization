...
def _package_root() -> Path:
    ...
    for parent in Path(__file__).resolve().parents:
        ...
        if runtime.is_dir():
            return runtime
    # Allow module import for non-XFeat workflows and tests. MegaLoc/XFeat model
    # loading still fails with its native file-not-found error if the cache was
    # not installed.
    return Path(__file__).resolve().parents[2] / "執行環境"
