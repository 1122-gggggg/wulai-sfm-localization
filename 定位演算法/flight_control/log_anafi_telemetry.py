...
def run_log(*, ip: str, controller: str, secs: float, hz: float, out: Path,
    ...
    if with_video:
        ...
        try:
            ...
            print(f"[log] video grabber failed (telemetry continues): {e}", flush=True)
    ...
    try:
        with out.open("w", encoding="utf-8") as f:
            ...
            while time.monotonic() < t_end:
                ...
                if first_keys is None:
                    first_keys = [k for k in row if not k.startswith("_") and k not in
                                  ("t_unix", "t_mono", "t_iso")]
                    print(f"[log] first sample channels present: "
                          f"{len(row.get('_channels_ok', []))} / {len(messages)}", flush=True)
                    for lab in row.get("_channels_ok", [])[:20]:
                        print(f"  OK  {lab}", flush=True)
                    miss = row.get("_channels_missing", [])
                    if miss:
                        print(f"[log] missing on first sample ({len(miss)}): "
                              f"{', '.join(miss[:12])}...", flush=True)
                ...
                if n % max(1, int(hz)) == 0:
                    ...
                    spd = row.get("piloting.speed")
                    bat = row.get("common.battery") or row.get("battery.capacity")
                    print(f"[log] n={n} att={att} alt={alt} spd={spd} bat={bat}", flush=True)
        ...
        try:
            ...
            pass
    print(f"[log] wrote {n} samples -> {out}", flush=True)
    return out
