...
**Updated: 2026-08-12 — mission-selection authorization, sparse-cloud proximity interlock, and agent prohibition**
...
- 真機只接受經 resolver 驗證的 `SFM_MISSION_SELECTION`；不得直接以 site profile
  啟動飛行。隨附的 `river_site_b0_p116_p117_localization.json` 通過元件與定位品質
  後即可地面定位；沒有 route，因此不是 `flight_ready`。AUTO 仍須由
  操作員完成當次四步 preflight 並親自按下；ANAFI 羅盤校正完成且韌體回讀有效後，
  第一步自動通過。
